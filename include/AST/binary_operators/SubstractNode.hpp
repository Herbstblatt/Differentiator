...
namespace expression {
    template<typename T>
    class SubtractNode : public BinaryOpNode<T> {
    public:
        using BinaryOpNode<T>::BinaryOpNode;

        SubtractNode<T>* clone_with(std::unique_ptr<INode<T>> left_operand, std::unique_ptr<INode<T>> right_operand) const override {
            return new SubtractNode(std::move(left_operand), std::move(right_operand));
        }
        ...
        INode<T>* differentiate(std::string by) const override {
            ...
            return new SubtractNode(std::move(left), std::move(right));
        }
    };
};